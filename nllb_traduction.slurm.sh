#!/bin/bash
#SBATCH --job-name=nllb_traduction         # Job name
#SBATCH --output=logs/log_nllb_1.out          # Name of stdout output file. %x job_name, %j job_number
#SBATCH --error=logs/error_nllb_1.err       # Name of stdout output file. %x job_name, %j job_number
...
python nllb_traduction.py --input_path "dataset_cleaned.csv" --output_path "prova.csv"